...
class AuthController {
    async register({ request, response }) {
        ...
        try {
            const { name, fullname, email, password, type} = request.all()

            const user = await User.create({ name, fullname, email, password }, trx)
            const userRole = await Role.findBy('slug', type)
